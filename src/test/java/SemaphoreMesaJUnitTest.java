...
public class SemaphoreMesaJUnitTest {
  ...
  public void obtainSemaphoreImplementation()
    ...
    mySemImpl = SemaphoreImplClassMesa.class;
  }
  ...
  public void testUpThenDownShouldNotBlockMultipleThreads() throws Exception {
    ...
    for(int i = 1; i <= 40 ; i++) {
      Thread t = new UpThenDownThreadMesa(sem,this);
      t.start();
      allThreads.add(t);
    }
  ...
  public void testThatDownDoesBlock() throws Exception {
    SemaphoreInterface sem = createSemaphore();
    Thread t = new DowningThreadMesa(sem,this);
    t.start();
  ...
  public void testThatUpUnblocksBlockedThreadsWithUpperThreads() throws Exception {
    ...
    Thread.sleep(1000); // leaving some time for threads to start and block
    for(int i = 0; i < 100 ; i++) {
      Thread t = new UppingThreadMesa(sem,this);
      t.start();
      allThreads.add(t);
    }
  ...
  public void testThatReleaseAllWorksWithNoThreadWaiting() throws Exception {
    SemaphoreInterface sem = createSemaphore();
    // testing that release all and up do not interfere
    for(int i=0; i < 100; i++) {
  ...
  public void testStressWorkloadWithAllConcurrent() throws Exception {
    ...
    for(int i = 1; i <= 40 ; i++) {
      Thread t = new UpThenDownThreadMesa(createSemaphore(),this);
      t.start();
      allThreads.add(t);
    }
...
} // EndClass SemaphoreMesaJUnitTest

class TestingThreadMesa extends Thread {
  protected SemaphoreInterface mySemaphore;
  protected SemaphoreMesaJUnitTest myTestCase;

  public TestingThreadMesa(SemaphoreInterface aSemaphore, SemaphoreMesaJUnitTest aTestCase) {
    mySemaphore = aSemaphore;
    myTestCase  = aTestCase;
  }
} // EndClass TestingThread

class UpThenDownThreadMesa extends TestingThreadMesa {

  public UpThenDownThreadMesa(SemaphoreInterface aSemaphore, SemaphoreMesaJUnitTest aTestCase) {
...
class DowningThreadMesa extends TestingThreadMesa {

  public DowningThreadMesa(SemaphoreInterface aSemaphore, SemaphoreMesaJUnitTest aTestCase) {
    super(aSemaphore,aTestCase);
  }
...
class UppingThreadMesa extends TestingThreadMesa {

  public UppingThreadMesa(SemaphoreInterface aSemaphore, SemaphoreMesaJUnitTest aTestCase) {
    super(aSemaphore,aTestCase);
  }
